...
define(
    ...
    function (
        ...
        const TYPE = 'customrecord_pd_ai_url_config'
        const FIELDS = {
            id: { name: 'internalid' },
            subsidiary: { name: 'custrecord_pd_ai_din_subsidiaria', type: 'list' },
            urlNext: { name: 'custrecord_pd_ai_din_url_next' },
            configurationType: { name: 'custrecord_pd_ai_din_configuration_type' }
        };

        function getBySubsidiaryAndConfigType({ subsidiaryId, configType }) {
            return search_util.first({
                type: TYPE,
                columns: FIELDS,
                query: search_util
                    .where(search_util.query(FIELDS.subsidiary, 'anyof', subsidiaryId))
                    .and(search_util.query(FIELDS.configurationType, 'is', configType))
            });
        };
        ...
        function set(options) {
            ...
            _documentImportation[FIELDS.configurationType.name] = options?.data?.configurationType;
        ...
        return {
            getBySubsidiaryAndConfigType: getBySubsidiaryAndConfigType,
            create: create,
            save: save,
            set: set,
            setSubmit: setSubmit
